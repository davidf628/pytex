...
\newquestion
%variables
%a = nonzerorrand(-2.80, 2.80, 0.01)
%b = nonzerorrand(-2.80, 2.80, 0.01)
...
\begin{enumerate}[a.]
    \item \(P(z < @a@) =\) \iskey{$\text{normalcdf}(-\infty, @a@) = @sol_a@$}
    \item \(P(z > @b@) =\) \iskey{$\text{normalcdf}(@sol_b@, \infty) = @sol_b@$}
