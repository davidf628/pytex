...
%{
% n = rand(150, 400)
% p = rrand(.15, .35, .01) where ((n * p > 5) and (n * (1 - p) > 5))
% q = 1 - p
% s_err = sqrt(p * q / n)
% lb = round(p - 3 * s_err, 4)
% ub = round(p + 3 * s_err, 4)
% phat = rrand(lb, ub, 0.0001)
% x = round(phat * n)
% phat = x / n
% z = (phat - p) / s_err
% pval = 2 * (1 - normalcdf(abs(z), 1E99))
%} where (pval > 0.005) and (pval < 0.15)
% a = randfrom([0.01, 0.05, 0.1])
% apct = round(a * 100)
% ppct = round(p * 100)
% qpct = round(q * 100)
% decision = "Reject $H_0$" if pval <= a else "Fail to reject $H_0$"
% sufficient = "sufficient" if pval <= a else "insufficient"
%end
